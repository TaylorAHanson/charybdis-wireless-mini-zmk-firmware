...
*   **Transition to Built-in Zephyr Driver & SPI_HALF_DUPLEX:** Realizing the hardware is actually wired perfectly (no offset), we discarded the `badjeff` module and the physical hardware hacks in favor of the built-in Zephyr `pixart,pmw3610` driver. This modern driver natively supports the `duplex = <2048>;` (`SPI_HALF_DUPLEX`) property, which dynamically disables the nRF52's TX driver during dummy reads. This theoretically eliminates the need for the resistor/diode hack entirely, allowing us to map both `MOSI` and `MISO` cleanly to `P0.17`.
