...
*   **The Hardware Disconnect Discovery:** We attempted to map both MOSI and MISO to `P0.17` without a resistor. Surprisingly, this resulted in an `0xFF` read (with the internal pull-up enabled), rather than the expected overpower by the ORC. This proved a critical physical behavior: the `nrfx_spim` hardware completely disconnects the `MOSI` pin during dummy reads (when the TX buffer is NULL). While this means a resistor is theoretically unnecessary to prevent contention during reads, the `MOSI` driver is still required to send the `0x00` address during the write phase. If the sensor is deaf or the clock is failing, the line is left floating during the read, which the internal pull-up drags to `0xFF`. The issue is now isolated to a potential `SCLK` hardware defect or a `t_SRAD` timing violation (which we are testing by dropping the frequency to 10kHz).
