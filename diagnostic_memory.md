...
*   **The Hardware Disconnect Discovery:** We attempted to map both MOSI and MISO to `P0.17` without a resistor. Surprisingly, this resulted in an `0xFF` read (with the internal pull-up enabled), rather than the expected overpower by the ORC. This proved a critical physical behavior: the `nrfx_spim` hardware completely disconnects the `MOSI` pin during dummy reads (when the TX buffer is NULL). 
*   **The SuperMini Pinout Discovery:** The user provided an image proving they are using a **SuperMini NRF52840**, not a standard nice!nano. The SuperMini has a fundamentally different physical pinout for its analog pins. `P0.28` (which the devicetree was using for the MOSI driver) does not exist on the edge headers. When the user was instructed to solder the resistor to the "5th pin down, right side" (assuming it was `P0.28`), they actually soldered it to `VCC`. This perfectly explains why the sensor was deaf during the resistor hack: it was receiving constant 3.3V power on its data line instead of the clock commands, while the microcontroller was screaming into a physically disconnected pad inside the chip. The fix is to map `SPIM_MOSI` to `P0.31` (6th pin down, right side) and bridge the diode across to `P0.17` (6th pin down, left side).
