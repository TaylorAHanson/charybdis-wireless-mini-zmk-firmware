...
*   **The Floating MISO Discovery:** After installing the 2.2kΩ resistor, the sensor returned `0x00`. To diagnose if the line was physically trapped at 0V, an internal `bias-pull-up` (~13kΩ) was added to `P0.17`. The read immediately flipped to `0xFF`. Since the user confirmed the 2.2kΩ resistor has perfect continuity, this proves the `MOSI` pin (`P0.28`) must be floating/disconnected by the hardware during the dummy read, leaving `P0.17` entirely dependent on the sensor. Because the sensor is currently deaf/unresponsive, it leaves the line high-Z, causing it to drift to 0V (without pull-up) or 3.3V (with pull-up). The root issue is now isolated to the sensor not receiving or understanding the initial clock/command.
